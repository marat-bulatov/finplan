...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//+mdfc
	Если МетаданныеСправочникаСФайлами.ТабличныеЧасти.Найти("КонтрольКопий") = Неопределено Тогда
		Элементы.КонтрольКопий.Видимость = Ложь;
	КонецЕсли;
...
Процедура НастроитьДинамическийСписок()
	...
	Если ТипЗнч(Параметры.ВладелецФайла) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
		...
		ЗапросВладельцев = Новый Запрос("ВЫБРАТЬ
		|	ДоговорыКонтрагентовПрисоединенныеФайлыВладельцыФайла.Ссылка.ВладелецФайла КАК ВладелецФайла
		|ИЗ
		|	Справочник.ДоговорыКонтрагентовПрисоединенныеФайлы.ВладельцыФайла КАК ДоговорыКонтрагентовПрисоединенныеФайлыВладельцыФайла
		|ГДЕ
		|	ДоговорыКонтрагентовПрисоединенныеФайлыВладельцыФайла.ВладелецФайла = &ВладелецФайла
		|СГРУППИРОВАТЬ ПО
		|	ДоговорыКонтрагентовПрисоединенныеФайлыВладельцыФайла.Ссылка.ВладелецФайла"); // Вложенным запросом в данном случае быстрее
		
		
		ЗапросВладельцев.УстановитьПараметр("ВладелецФайла", Параметры.ВладелецФайла);
		Выборка = ЗапросВладельцев.Выполнить().Выбрать();
		
		МассивВладельцев = Новый Массив;
		Пока Выборка.Следующий() Цикл
			
			МассивВладельцев.Добавить(Выборка.ВладелецФайла);
			
		КонецЦикла;
		МассивВладельцев.Добавить(Параметры.ВладелецФайла);
		
		СвойстваСписка.ТекстЗапроса = СтрЗаменить(СвойстваСписка.ТекстЗапроса, "Файлы.ВладелецФайла = &ВладелецФайлов", "Файлы.ВладелецФайла В (&ВладелецФайлов)");
		Список.Параметры.УстановитьЗначениеПараметра("ВладелецФайлов", МассивВладельцев);
		ОбщегоНазначения.УстановитьСвойстваДинамическогоСписка(Элементы.Список, СвойстваСписка); // Повторение - мать учения
...
//+mdfc
&НаКлиенте
Процедура КонтрольКопий(Команда)
	
	Если Элементы.Список.ТекущиеДанные <> Неопределено Тогда
		
		ПараметрыФормы = Новый Структура("Ссылка", Элементы.Список.ТекущиеДанные.Ссылка);
		ОткрытьФорму("Обработка.РаботаСФайлами.Форма.ФормаКонтрольКопий", ПараметрыФормы);
...
Процедура Ресурсы(Команда)
	...
	Если Элементы.Список.ТекущиеДанные <> Неопределено Тогда
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ВладелецФайла", Элементы.Список.ТекущиеДанные.Ссылка);
		ПараметрыФормы.Вставить("ЗаголовокФормы", "Ресурсы " + Элементы.Список.ТекущиеДанные.Ссылка);
