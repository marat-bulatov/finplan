...
Функция ОбновитьБанкиИзКлассификатора(Знач СписокБанков = Неопределено, Знач ОбластьДанных) Экспорт
	...
	|		Справочник.КлассификаторБанков КАК Банки
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ИзмененныеБанки КАК ВТ_ИзмененныеБанки
	|			ПО (НЕ Банки.ДеятельностьПрекращена)
...
Функция ПодобратьБанкИзКлассификатора(Знач СсылкиБанков, ИгнорироватьРучноеИзменение = Ложь) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(Банки.Ссылка, ЗНАЧЕНИЕ(Справочник.КлассификаторБанков.ПустаяСсылка)) КАК Банк,
	...
	|
	|ВЫБРАТЬ
	|	ЕСТЬNULL(Банки.Ссылка, ЗНАЧЕНИЕ(Справочник.КлассификаторБанков.ПустаяСсылка)),
...
Функция ЗагрузитьПоставляемыйКлассификаторБанков(ПутьКФайлу) Экспорт
	...
	Возврат Обработки.ЗагрузкаКлассификатораБанков.ЗагрузитьДанныеИзФайла(ПутьКФайлу);
...
Функция НайтиБанкПоСВИФТБИК(СВИФТБИК)
	...
	Банк = Справочники.КлассификаторБанков.СсылкаНаБанкПоСВИФТБИК(СВИФТБИК);
	Если ЗначениеЗаполнено(Банк) Тогда
		Возврат Банк;
	КонецЕсли;
...
Функция СоздатьОбновитьИностранныйБанкПоСВИФТБИК(ДанныеЗаполнения, Банк = Неопределено)
	...
	БанкОбъект.Страна = Справочники.КлассификаторБанков.СтранаПоSWIFT(БанкОбъект.СВИФТБИК);
	...
	Для инд = 0 По Ссылки.ВГраница() Цикл
		...
		Если Банк.Пустая() Тогда
			Если ПараметрыОбъект.ЭтоРегион Тогда
				БанкОбъект = Справочники.КлассификаторБанков.СоздатьГруппу();
			Иначе
				БанкОбъект = Справочники.КлассификаторБанков.СоздатьЭлемент();
			КонецЕсли;
		Иначе
		...
		Если НЕ ПустаяСтрока(ПараметрыОбъект.РодительКод) И НЕ ЗначениеЗаполнено(ПараметрыОбъект.Родитель) Тогда
			Регион = Справочники.КлассификаторБанков.СсылкаНаБанк(ПараметрыОбъект.РодительКод, Истина);
