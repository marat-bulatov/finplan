...
Функция ПолучитьРеквизитыБанкаИзСправочника(СсылкаНаБанк) Экспорт
	...
	ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаБанк,"Код,СВИФТБИК,Наименование,ДеятельностьПрекращена");
	...
	РеквизитыБанка.ДеятельностьПрекращена = ЗначенияРеквизитов.ДеятельностьПрекращена;
...
Функция ПолучитьРеквизитыБанкаИзКлассификатора(СсылкаНаБанк) Экспорт
	...
	Если РеквизитыБанка.Ссылка = Справочники.КлассификаторБанков.ПустаяСсылка() Тогда
...
Функция НоваяСтруктураРеквизитовБанка()
	...
	СтруктураРеквизитов.Вставить("Ссылка", Справочники.КлассификаторБанков.ПустаяСсылка());
...
Функция СведенияОВалютномУчете() Экспорт
	...
	Результат.Вставить("ВалютныйУчетИспользуется",         Истина);
	Результат.Вставить("ИзменениеВалютногоУчетаРазрешено", Ложь);
...
//+mdfc
Функция ВалютаБанковскогоСчета(БанковскийСчет) Экспорт
	...
	Если КодВалюты = БанковскиеПравила.КодРубляВНомереСчета() Тогда
		Возврат ОбщегоНазначенияБПВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета();
	Иначе
		Возврат БанковскиеСчетаВызовСервера.ПолучитьВалютуПоКоду(КодВалюты);
	КонецЕсли;
