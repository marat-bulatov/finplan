...
Процедура РассчитатьСуммыНаСервере()
	...
	//Если ТаблицаВыписка.Количество() = 0 Тогда	
	//	
	//	Док = Документы.Выписка.Выбрать(НАчалоДня(Дата) , КонецДня(Дата) );
	//	
	//	ПоказатьПриходОтДругихОрганизаций = Неопределено;
	//	
	//	Пока Док.Следующий() Цикл
	//		
	//		Если ПоказатьПриходОтДругихОрганизаций = Ложь Тогда
	//			Прервать;
	//		КонецЕсли; 
	//		
	...
	//		КонецЕсли;
	//		
	//		Для Каждого Стр Из Док.ТаблицаВыписка Цикл
	//			
	//			Если Стр.Контрагент = Фирма И Стр.СчетКонтрагента = Счет И НЕ(Стр.ВидДвиженияДС = Перечисления.ВидДвиженияДенежныхСредств.Приход) Тогда 
	//				Если ПоказатьПриходОтДругихОрганизаций = Неопределено Тогда
	//					Если Вопрос("Показать приход от других организаций?", РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Нет Тогда
	//						ПоказатьПриходОтДругихОрганизаций = Ложь;
	//						Прервать; 
	//					Иначе 
	//						ПоказатьПриходОтДругихОрганизаций = Истина;
	//					КонецЕсли;
	//				КонецЕсли;
	//				НС = ТаблицаВыписка.Добавить();
	//				НС.GUIDСтрокиВыписки = Новый УникальныйИдентификатор;
	//				НС.ВидДвиженияДС 	 = Перечисления.ВидДвиженияДенежныхСредств.Приход; 
	//				НС.Контрагент		 = Док.Фирма; 
	//				НС.СчетКонтрагента 	 = Док.Счет;
	//				НС.Приход 			 = Стр.Расход;
	//				НС.НазначениеПлатежа = Стр.НазначениеПлатежа;
	//				//ЭлементыФормы.ТаблицаВыписка.ТекущаяСтрока.Контрагент = Справочники.Фирмы.ПустаяСсылка();
	//				//ЭлементыФормы.ТаблицаВыписка.ТекущаяСтрока.Контрагент = Стр.Фирма; 
	//				//ЭлементыФормы.ТаблицаВыписка.ТекущаяСтрока.СчетКонтрагента = Справочники.Счета.ПустаяСсылка(); 
	//				//ЭлементыФормы.ТаблицаВыписка.ТекущаяСтрока.СчетКонтрагента = Стр.Счет;
	//				//ЭлементыФормы.ТаблицаВыписка.ТекущаяСтрока.Приход = Стр.Расход;
	//				//ЭлементыФормы.ТаблицаВыписка.ТекущаяСтрока.НазначениеПлатежа = Стр.НазначениеПлатежа;
	//			КонецЕсли;
	//			
	//		КонецЦикла;
	//	КонецЦикла;
	...
	//|	ЕСТЬNULL(Регистр.СуммаОстаток, 0) КАК СуммаОстаток
	//|ИЗ
	//|	РегистрНакопления.ОстаткиДенСредств.Остатки(
	//|			&МоментВремени, 
	//|			Фирма = &Фирма
	//|				И Счета = &Счет) КАК Регистр"; 
	//Запрос = Новый Запрос(ТекстЗапроса);
	//Запрос.УстановитьПараметр("МоментВремени", Новый МоментВремени(Ссылка.Дата, Ссылка) );
	//Запрос.УстановитьПараметр("Фирма", 	Фирма);
	//Запрос.УстановитьПараметр("Счет", 	Счет);
	//р = Запрос.Выполнить().Выгрузить();
	//
	//
	//Если НЕ внВалютный Тогда
	//	ОстатокНаНачалоДня = ?(р.Количество() >0, р.Получить(0).СуммаОстаток, 0);
	//	ОстатокНаКонецДня = ОстатокНаНачалоДня + ТаблицаВыписка.Итог("Приход") - ТаблицаВыписка.Итог("Расход");
	//Иначе 
	//	КурсВал = общПолучитьКурс(Валюта, Дата);	
	//	ОстатокНаНачалоДня = ?(р.Количество() >0, р.Получить(0).СуммаОстаток*КурсВал, 0);
	//	ОстатокНаНачалоДняВал = ?(р.Количество() >0, р.Получить(0).СуммаОстаток, 0);
	//	ОстатокНаКонецДня = ОстатокНаНачалоДня + ТаблицаВыписка.Итог("Приход") -ТаблицаВыписка.Итог("Расход");
	//	// Новый расчет итоговой суммы
	//	ОстатокНаКонецДняВал = ОстатокНаНачалоДняВал;
	//	Для каждого стр Из ТаблицаВыписка Цикл			
	//		Если стр.Приход>0 Тогда 
	//			ОстатокНаКонецДняВал = ОстатокНаКонецДняВал + стр.ВалСумма;
	//		Иначе
	//			ОстатокНаКонецДняВал = ОстатокНаКонецДняВал-стр.ВалСумма;
	//		КонецЕсли;
	//		
	//	КонецЦикла;
	//	
	//	ЭлементыФормы.ОстатокНаНачалоДняВал.Значение = ФРМ(ОстатокНаНачалоДняВал, 1);
	//	ЭлементыФормы.ОстатокНаКонецДняВал.Значение = ФРМ(ОстатокНаКонецДняВал, 1);
	//	
	//	//		ОстатокНаКонецДняВал = (ОстатокНаНачалоДняВал + ТаблицаВыписка.Итог("Приход") /?(КурсВал>0, КурсВал, 1) ) -(ТаблицаВыписка.Итог("Расход") /?(КурсВал>0, КурсВал, 1) );
	//КонецЕсли;
	//
	//ЭлементыФормы.ИтогОстатокНаНачалоДня.Значение = ФРМ(ОстатокНаНачалоДня, 1);
	//ЭлементыФормы.ИтогОстатокНаКонецДня.Значение = ФРМ(?(ОстатокНаКонецДня < 0.01, 0, ОстатокНаКонецДня), 1);
...
Процедура ЗаполнитьИзФинансовогоПланаЗавершениеНаСервере()
	...
	Пока ВыборкаПланированиеРасход.Следующий() Цикл
		...
		ДополнительнаяОбработкаСтроки(НоваяСтрока);
...
Процедура ДополнительнаяОбработкаСтроки(СтрокаОбработки, ДополнительныеСведения = Неопределено)
	
	// Зарплата на к/к
	// Проверим RegExp
	
	НазначениеПлатежа = СтрокаОбработки.НазначениеПлатежа;
	НаименованиеФирмы = Объект.Организация.Наименование;
	НаименованиеКонтрагента = ?(ЗначениеЗаполнено(СтрокаОбработки.Контрагент), СтрокаОбработки.Контрагент.Наименование, "");
	БанкСтрокой = СокрЛП(Объект.СчетОрганизации.Банк);
	...
	Если RegularExpressions.ТестRegExp(СтрокаОбработки.НазначениеПлатежа, "(Зарплата\s+на\s+к/к\s+\d+/\d+)") Тогда
		
		СтрокаОбработки.КатегорияОперации = Справочники.КатегорииОпераций.НайтиПоНаименованию("(З/п) основная часть", Истина);
		СтруктураДоработки.Вставить("ЦФУ", Справочники.ЦФУ.НайтиПоНаименованию("M-Consult", Истина));
		
		RegExp.Pattern = "(Зарплата\s+на\s+к/к\s+\d+/\d+)"; 
		НазначениеПлатежа = RegExp.Execute(СтрокаОбработки.НазначениеПлатежа).Item(0).Value;
		
		RegExp.Pattern = "(Зарплата\s+на\s+к/к\s+)";
		НазначениеПлатежа = RegExp.Replace(НазначениеПлатежа, "");
		
		RegExp.Pattern = "(/\d+)";
		МесяцПлатежа = RegExp.Replace(НазначениеПлатежа, "");
		
		RegExp.Pattern = "(^\d+/)";
		ГодПлатежа = RegExp.Replace(НазначениеПлатежа, "");
		
		Если ГодПлатежа <> "" И МесяцПлатежа <> "" Тогда
			
			Попытка
				ДатаПлатежа = Дата(ГодПлатежа + МесяцПлатежа + "01");
				...
				|ГДЕ
				|	Периоды.ПоследнийДеньМесяца МЕЖДУ &ДатаНачала И &ДатаОкончания";
				
				ЗапросПериодов.УстановитьПараметр("ДатаНачала", НачалоМесяца(ДатаПлатежа));
				ЗапросПериодов.УстановитьПараметр("ДатаОкончания", КонецМесяца(ДатаПлатежа));
				
				РезультатЗапроса = ЗапросПериодов.Выполнить();
				
				ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
				Если ВыборкаДетальныеЗаписи.Следующий() Тогда
					СтруктураДоработки.Вставить("Период", ВыборкаДетальныеЗаписи.Ссылка);
				КонецЕсли;
				
			Исключение
				
				Сообщить("Невозможно распознать дату, поле ""ПЕРИОД"" не будет заполнен автоматически... :(");
				
			КонецПопытки;
			
		КонецЕсли;
		
	ИначеЕсли RegularExpressions.ТестRegExp(СтрокаОбработки.НазначениеПлатежа, "(^(\s+){0,1}комиссия\s+банка(\s+){0,1}$)") Тогда	
		СтрокаОбработки.КатегорияОперации = Справочники.КатегорииОпераций.НайтиПоНаименованию("Комиссия банка", Истина);
		
	ИначеЕсли RegularExpressions.ТестRegExp(СтрокаОбработки.НазначениеПлатежа, "(НСиПЗ\s+\d+/\d+)") Тогда	
		СтруктураДоработки.Вставить("ВидНалогов", Справочники.ЗначенияСубконтоУправленческогоУчета.НайтиПоНаименованию("СВВФ (страх.взносы в ВФ)", Истина,, ПланыВидовХарактеристик.РеквизитыУправленческогоУчета.ТипСубконтоВидыНалогов));
		СтрокаОбработки.КатегорияОперации = Справочники.КатегорииОпераций.НайтиПоНаименованию("Оплата налогов", Истина);
		
	ИначеЕсли RegularExpressions.ТестRegExp(СтрокаОбработки.НазначениеПлатежа, "(СВВФ\s+\d+/\d+)") Тогда	
		СтруктураДоработки.Вставить("ВидНалогов", Справочники.ЗначенияСубконтоУправленческогоУчета.НайтиПоНаименованию("СВВФ (страх.взносы в ВФ)", Истина,, ПланыВидовХарактеристик.РеквизитыУправленческогоУчета.ТипСубконтоВидыНалогов));
		СтрокаОбработки.КатегорияОперации = Справочники.КатегорииОпераций.НайтиПоНаименованию("Оплата налогов", Истина);
		
	ИначеЕсли RegularExpressions.ТестRegExp(СтрокаОбработки.НазначениеПлатежа, "(НДФЛ\s+\d+/\d+)") Тогда	
		СтруктураДоработки.Вставить("ВидНалогов", Справочники.ЗначенияСубконтоУправленческогоУчета.НайтиПоНаименованию("НДФЛ", Истина,, ПланыВидовХарактеристик.РеквизитыУправленческогоУчета.ТипСубконтоВидыНалогов));
		СтрокаОбработки.КатегорияОперации = Справочники.КатегорииОпераций.НайтиПоНаименованию("Оплата налогов", Истина);
		
	ИначеЕсли RegularExpressions.ТестRegExp(СтрокаОбработки.НазначениеПлатежа, "(Суточн)")
		И СтрокаОбработки.КатегорияОперации = Справочники.КатегорииОпераций.НайтиПоНаименованию("Командировочные расходы") Тогда	
	...
	ИначеЕсли RegularExpressions.ТестRegExp(СтрокаОбработки.НазначениеПлатежа, "(Оплата за товары нар)") Тогда
		
		СтрокаОбработки.КатегорияОперации = Справочники.КатегорииОпераций.НайтиПоНаименованию("Взаиморасчеты с ЦУМом/ДЛТ, поступление");
		СтруктураДоработки.Вставить("КатегорияВзаиморасчетов", Справочники.ЗначенияСубконтоУправленческогоУчета.НайтиПоНаименованию("ЦУМ, расчеты за товар Россия", Истина,, ПланыВидовХарактеристик.РеквизитыУправленческогоУчета.ТипСубконтоВидыКомандировочныхРасходов));
		
	ИначеЕсли RegularExpressions.ТестRegExp(СтрокаОбработки.НазначениеПлатежа, "(?=^Перевод\s+средств\s+за\s+покупку)(?=.*СБЕРБАНКМАРКЕТС)") Тогда
		СтрокаОбработки.КатегорияОперации = Справочники.КатегорииОпераций.НайтиПоНаименованию("Конвертация р/сч Х, отправка денег");
		
	ИначеЕсли RegularExpressions.ТестRegExp(СтрокаОбработки.НазначениеПлатежа, "(Аксапта)") Тогда
		СтрокаОбработки.КатегорияОперации = Справочники.КатегорииОпераций.НайтиПоНаименованию("Невыясненные переводы (выплата)");
	...
	ИначеЕсли RegularExpressions.ТестRegExp(НаименованиеКонтрагента, "(УФК по г.Москве \(ФТС России\))")
		...
		СтрокаОбработки.КатегорияОперации = Справочники.КатегорииОпераций.НайтиПоНаименованию("Платежи по таможне", Истина);
		СтруктураДоработки.Вставить("ВидТаможенныхПлатежей", Справочники.ЗначенияСубконтоУправленческогоУчета.НайтиПоНаименованию("Платежи в бюджет", Истина,, ПланыВидовХарактеристик.РеквизитыУправленческогоУчета.ТипСубконтоВидыТаможенныхПлатежей));
		
		Если RegularExpressions.ТестRegExp(НаименованиеФирмы, "(ТОП МОТО)") ИЛИ RegularExpressions.ТестRegExp(НаименованиеФирмы, "(Маранелло)") Тогда
			СтруктураДоработки.Вставить("ЦФУ", Справочники.ЦФУ.НайтиПоНаименованию("Логистика-Авто мото", Истина));
		ИначеЕсли RegularExpressions.ТестRegExp(НаименованиеФирмы, "(М Фэшн)") Тогда
			СтруктураДоработки.Вставить("ЦФУ", Справочники.ЦФУ.НайтиПоНаименованию("Логистика-Одежда", Истина));
		ИначеЕсли RegularExpressions.ТестRegExp(НаименованиеФирмы, "(Меркури Мода)") Тогда
			СтруктураДоработки.Вставить("ЦФУ", Справочники.ЦФУ.НайтиПоНаименованию("Логистика-Одежда", Истина));
		ИначеЕсли RegularExpressions.ТестRegExp(НаименованиеФирмы, "(Меркури Классик)") Тогда
	...
	ИначеЕсли RegularExpressions.ТестRegExp(НаименованиеКонтрагента, "(УФК по г. Москве \(Государственная инспекция пробирного надзора по г. Москве и Московской области\))") Тогда // RegularExpressions.ТестRegExp(СтрокаОбработки.НазначениеПлатежа, "(Госпошлина)")
		...
		СтрокаОбработки.КатегорияОперации = Справочники.КатегорииОпераций.НайтиПоНаименованию("Сертификация товаров", Истина);
	...
	ИначеЕсли RegularExpressions.ТестRegExp(НаименованиеКонтрагента, "(MERCURY DISTRIBUTION S\.A\.)") Тогда
			
			СтрокаОбработки.КатегорияОперации = Справочники.КатегорииОпераций.НайтиПоНаименованию("Перевод Х в БМВ напрямую (р/сч Х => БМВ, отправка)", Истина);
			
			Если ТипЗнч(ДополнительныеСведения) = Тип("Структура")
				И ДополнительныеСведения.Свойство("Счет") Тогда
				
				//СтрокаОбработки.СуммаВВалютеЗаявки = СтрокаОбработки.Расход;
				СтрокаОбработки.ВалСумма = СтрокаОбработки.Расход;
				
				//Если НаименованиеФирмы = "Меркури Классик" Тогда
				//	
				//	Если ДополнительныеСведения.Счет.ВидСчета = Перечисления.ВидСчета.ТекущийEUR Тогда
				//		СтруктураДоработки.Вставить("СчетПолучатель", Справочники.ЗначенияСубконтоУправленческогоУчета.НайтиПоНаименованию("Счет Mrc Distrib SA Gene_EUR (UBS)", Истина,, ПланыВидовХарактеристик.РеквизитыУправленческогоУчета.ТипСубконтоСчетаЗаРубежом));
				//	ИначеЕсли ДополнительныеСведения.Счет.ВидСчета = Перечисления.ВидСчета.ТекущийCHF Тогда
				//		СтруктураДоработки.Вставить("СчетПолучатель", Справочники.ЗначенияСубконтоУправленческогоУчета.НайтиПоНаименованию("Счет Mrc Distrib SA Gene_CHF (UBS)", Истина,, ПланыВидовХарактеристик.РеквизитыУправленческогоУчета.ТипСубконтоСчетаЗаРубежом));
				//	ИначеЕсли ДополнительныеСведения.Счет.ВидСчета = Перечисления.ВидСчета.ТекущийUSD Тогда
				//		СтруктураДоработки.Вставить("СчетПолучатель", Справочники.ЗначенияСубконтоУправленческогоУчета.НайтиПоНаименованию("Счет MD SA (Swiss)_USD (CS)", Истина,, ПланыВидовХарактеристик.РеквизитыУправленческогоУчета.ТипСубконтоСчетаЗаРубежом));
				//	КонецЕсли;
				//	
				//Иначе
				//	
				//	Если ДополнительныеСведения.Счет.ВидСчета = Перечисления.ВидСчета.ТекущийEUR Тогда
				//		СтруктураДоработки.Вставить("СчетПолучатель", Справочники.ЗначенияСубконтоУправленческогоУчета.НайтиПоНаименованию("Счет MD SA (Swiss)_EUR (CS)", Истина,, ПланыВидовХарактеристик.РеквизитыУправленческогоУчета.ТипСубконтоСчетаЗаРубежом));
				//	ИначеЕсли ДополнительныеСведения.Счет.ВидСчета = Перечисления.ВидСчета.ТекущийCHF Тогда
				//		СтруктураДоработки.Вставить("СчетПолучатель", Справочники.ЗначенияСубконтоУправленческогоУчета.НайтиПоНаименованию("Счет MD SA (Swiss)_CHF (CS)", Истина,, ПланыВидовХарактеристик.РеквизитыУправленческогоУчета.ТипСубконтоСчетаЗаРубежом));
				//	ИначеЕсли ДополнительныеСведения.Счет.ВидСчета = Перечисления.ВидСчета.ТекущийUSD Тогда
				//		СтруктураДоработки.Вставить("СчетПолучатель", Справочники.ЗначенияСубконтоУправленческогоУчета.НайтиПоНаименованию("Счет MD SA (Swiss)_USD (CS)", Истина,, ПланыВидовХарактеристик.РеквизитыУправленческогоУчета.ТипСубконтоСчетаЗаРубежом));
				//	КонецЕсли;
				//	
	...
	ИначеЕсли (RegularExpressions.ТестRegExp(НазначениеПлатежа, "(ДЕПОЗИТ\s+)") ИЛИ RegularExpressions.ТестRegExp(НазначениеПлатежа, "(ОВЕРНАЙТ\s+)")) Тогда
		                                                                                     
		СтрокаОбработки.КатегорияОперации = Справочники.КатегорииОпераций.НайтиПоНаименованию("Перевод денег на депозит", Истина);
		
		СоответствиеБанковКонтрагентов = Новый Соответствие;
		СоответствиеБанковКонтрагентов.Вставить("ВТБ", "Банк ВТБ");
		СоответствиеБанковКонтрагентов.Вставить("Сбербанк", "Сбербанк");
		СоответствиеБанковКонтрагентов.Вставить("Альфа-Банк", "Альфа-Банк");
		СоответствиеБанковКонтрагентов.Вставить("СРБ", "СЕРВИС РЕЗЕРВ БАНК");
		
		
		НаименованиеКонтрагентаБанка = СоответствиеБанковКонтрагентов.Получить(БанкСтрокой);
		Если НаименованиеКонтрагентаБанка <> Неопределено Тогда
			СтрокаОбработки.Контрагент = Справочники.Контрагенты.НайтиПоНаименованию(НаименованиеКонтрагентаБанка, Истина);
		КонецЕсли;
		
		// Ищем депозит
		Если СокрЛП(НаименованиеФирмы) = "М Стиль" И БанкСтрокой = "СРБ" Тогда
			НаименованиеДепозита = "Депозит М Стиль (СРБ) однодневный";
		Иначе
			НаименованиеДепозита = "Депозит " + СокрЛП(НаименованиеФирмы) + " (" + БанкСтрокой + ")";
		КонецЕсли;
	...
	ИначеЕсли RegularExpressions.ТестRegExp(НаименованиеКонтрагента, "(МГТС)")
		И RegularExpressions.ТестRegExp(НаименованиеФирмы, "Вечная память солдатам")
		И RegularExpressions.ТестRegExp(НазначениеПлатежа, "Оплата услуг связи за период") Тогда
		
		СтрокаОбработки.КатегорияОперации = Справочники.КатегорииОпераций.НайтиПоНаименованию("Услуги связи (тел. городской, e-net, оптоволокно)", Истина);
		СтруктураДоработки.Вставить("ЦФУ", Справочники.ЦФУ.НайтиПоНаименованию("Площадка Третьяковский проезд", Истина));
		
	ИначеЕсли RegularExpressions.ТестRegExp(НазначениеПлатежа, "Погашение\s+процентов\s+по\s+кредитному\s+договору") Тогда
		СтрокаОбработки.КатегорияОперации = Справочники.КатегорииОпераций.НайтиПоНаименованию("Проценты за кредит/депозит полученный (выплата)", Истина);
		
	ИначеЕсли RegularExpressions.ТестRegExp(НазначениеПлатежа, "Погашение\s+основного\s+долга\s+по\s+кредитному\s+договору") Тогда
		СтрокаОбработки.КатегорияОперации = Справочники.КатегорииОпераций.НайтиПоНаименованию("Погашение банковского кредита (краткосрочного)", Истина);
		
	КонецЕсли;
	
	//Если ЗначениеЗаполнено(СтрокаОбработки.КатегорияОперации) Тогда
	//	
	//	ЭлементыФормы.ТаблицаВыписка.ТекущаяСтрока = СтрокаОбработки;
	//	ДобавлениеКатегории(Ложь);
	//	
	//	Для Каждого КлючЗначение Из СтруктураДоработки Цикл
	//		
	//		Для Каждого СтрокаТЧ Из ТаблицаКатегорийОпераций.НайтиСтроки(Новый Структура("НомерСтрокиВыписки, Имя", СтрокаОбработки.НомерСтроки, КлючЗначение.Ключ)) Цикл
	//			СтрокаТЧ.Значение = КлючЗначение.Значение;	
	//		КонецЦикла;
	//			
	//	КонецЦикла;
	//	
