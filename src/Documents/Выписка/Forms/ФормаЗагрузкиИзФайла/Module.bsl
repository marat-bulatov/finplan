...
Функция ЗагрузитьДанныеНаСервере(МассивИнформации)
	...
	Для Каждого Информация Из МассивИнформации Цикл
		...
		Если Не ЗначениеЗаполнено(НоваяСтрока.СчетОрганизации) Тогда
			ОбщегоНазначения.СообщитьПользователю("Не найден расчетный счет организации " + НоваяСтрока.Организация + " номер " + НомерСчета + " БИК " + БИКБанкаОрганизации);
		КонецЕсли;
		...
		Пока ВыборкаПоУправленческомуУчету.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(ВыборкаПоУправленческомуУчету.КатегорияОпераций) И НЕ ЗначениеЗаполнено(НоваяСтрока.КатегорияОперации) Тогда
	...
	Если ЭтоИНН(ИНН) Тогда
		...
		Если Не ЗначениеЗаполнено(Контрагент) Тогда
			...
			Если ТипЗнч(РеквизитыКонтрагента) = Тип("Структура") И НЕ ЗначениеЗаполнено(РеквизитыКонтрагента.ОписаниеОшибки) Тогда
				...
				ОбщегоНазначения.СообщитьПользователю("Создан контрагент: " + НовыйКонтрагент);
...
Функция ТестRegExp(ТекстДляТестирования, ВыражениеRegExp)
	...
	Если RegExp = Неопределено Тогда
		RegExp = RegularExpressions.ПолучитьОбъектRegExp();
	КонецЕсли;
	Возврат RegularExpressions.ТестRegExp(ТекстДляТестирования, ВыражениеRegExp, RegExp);
...
Функция ПодстрокаRegExp(Текст, ВыражениеRegExp)
	...
	Возврат RegularExpressions.ПодстрокаRegExp(Текст, ВыражениеRegExp, RegExp);
